...
class CreateComputerView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            photo = request.FILES.get('photo-url', False)
            if photo:
                ...
                try:
                    comp_img_data = cloudinary.uploader.upload(photo, public_id=pub_id,
                                                               allowed_formats=['png, jpg, gif, bmp, psd, thm, tif'])
                    comp_img_details = ComputerImage(secure_url=comp_img_data['secure_url'],
                                                     public_id=comp_img_data['public_id'],
                                                     height=comp_img_data['height'], width=comp_img_data['width'],
                                                     original_filename=comp_img_data['original_filename'])
                    ...
                    if e.message == 'Invalid image file':
                        request.session['action'] = 'create'
                        return render_to_response('computer_creation.html', locals(),
...
class EditComputerView(View):
    def get(self, request, pk):
        ...
        form = ComputerCreationForm(instance=computer)
        request.session['action'] = 'edit'
        return render_to_response('computer_creation.html', locals(), context_instance=RequestContext(request))

    def post(self, request, pk):
        computer = Computer.objects.get(pk=pk)
        form = ComputerCreationForm(request.POST, instance=computer)
        if form.is_valid():
            form.save()
